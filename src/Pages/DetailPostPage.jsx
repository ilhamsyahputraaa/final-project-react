...
function DetailPostPage() {
  ...
  const handleDeletePost = (postId) => {
    if (
      window.confirm(
        "Are you sure you want to delete this Post? This change cannot be undone!"
      )
    ) {
      axios({
        method: "delete",
  ...
  const getLikesOfPost = () => {
    axios({
      ...
      url: `${
        import.meta.env.VITE_REACT_BASE_URL
      }/api/v1/explore-post?size=100&page=1`,
      headers: {
        apiKey: `${import.meta.env.VITE_REACT_API_KEY}`,
        Authorization: `Bearer ${jwtToken}`,
      },
      ...
      <div className="body d-flex row gap-5 BodyDetailPost">
          ...
          <Card style={{ width: "100%" }} id="PostCard">
            <Col id="UserPost">
              <Col id="UserName" className="d-flex row gap-4">
                <div id="AvatarImage">
                  <img
                    src={postUser.profilePictureUrl}
                    alt=""
                    className="AvatarPost"
                    onClick={() =>
                      window.location.assign(
                        `/profile?userId=${postDetail.userId}`
                      )
                    }
        ...
        <div className="detailPostComent SideBar col-lg-3 col-md-12 d-flex row gap-4 ">
          <Card id="PostCard" className="p-4 d-flex gap-4">
              ...
              <Col
                ...
                <div className="d-flex column gap-2 align-items-center">
                  <FontAwesomeIcon
                    icon={faHeart}
                    style={
                      !isLikes.isLike ? { color: "grey" } : { color: "red" }
                    }
                    onClick={() => {
                      isLikes.isLike
                        ? handleUnlikeButton(postDetail)
                        : handleLikeButton(postDetail);
                    }}
                  />
                ...
                <div className="d-flex column gap-2 align-items-center">
                  ...
                  <Col className="Likes p-0 m-0">
                    {commentCount[postDetail.id] ?? 0} Comments
                  </Col>
                </div>
              </Col>
            </Row>
            ...
            <Card.Text style={{ fontSize: 18 }}>{postDetail.caption}</Card.Text>
          </Card>
                  ...
                  <div className="d-flex gap-3">
                    <span>
                      <img
                        ...
                        onClick={() =>
                          window.location.assign(
                            `/profile?userId=${comment.user.id}`
                          )
                        }
                        style={{ objectFit: "cover", aspectRatio: "1/1" }}
                      />
